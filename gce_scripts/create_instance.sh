...
STARTUP="./gce_scripts/startup.sh"
if !ls ${trainer_package_path} &> /dev/null
then
  echo "STARTUP.sh NOT FOUND! Make sure you have './gce_scripts/startup.sh'  EXIT"
...
# Check image Source
if gcloud compute images list --filter=$BOOT_IMAGE | grep $BOOT_IMAGE &> /dev/null
then echo "Found boot image: $BOOT_IMAGE"
else
  echo "BOOT IMAGE $BOOT_IMAGE DOES NOT EXIST IN PROJECT $PROJ_NAME !!!"
